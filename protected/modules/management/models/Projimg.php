...
class Projimg extends CActiveRecord
{
    ...
    public function saveImages($allImg,$target){
        if (isset($allImg) && count($allImg) > 0) {
            ...
            if (!file_exists(Yii::getPathOfAlias('webroot').'/images/projects/'.$target.'/big')) {
                mkdir(Yii::getPathOfAlias('webroot').'/images/projects/'.$target.'/big', 0777, true);
                mkdir(Yii::getPathOfAlias('webroot').'/images/projects/'.$target.'/news', 0777, true);
                mkdir(Yii::getPathOfAlias('webroot').'/images/projects/'.$target.'/thumb', 0777, true);
            }
            ...
            foreach ($allImg as $image => $pic) {
                ...
                if ($pic->saveAs(Yii::getPathOfAlias('webroot').'/images/projects/'.$target.'/big/'.$finalName)) {

                    // add it to the main model now
                    $curModel->imgName = $finalName;
                    $curModel->proj_id = $target;
                    $curModel->id_image=NULL;
                    $curModel->isNewRecord=TRUE;

                    //create thumb
                    $originPath=Yii::getPathOfAlias('webroot').'/images/projects/'.$target.'/big/';
                    $pathNews=Yii::getPathOfAlias('webroot').'/images/projects/'.$target.'/news/';
                    $pathThumb=Yii::getPathOfAlias('webroot').'/images/projects/'.$target.'/thumb/';
